...
for idx = 1:length(N_values)
    ...
    for group = 1:num_groups
        ...
        U_group = fft(u_group, N);
        weights(group, :) = abs(U_group(1:floor(N/2))).^2;
    end
   
    mean_gw(idx, 1:N/2) = sum(gh .* weights, 1) ./ sum(weights, 1);
    freq = 2*pi*(0:N-1)/N;
    ...
    if idx ~= length(N_values)
        ...
        grid on;
        set(gca, 'XScale', 'log'); 
        legend([h_individual(1), h_mean], {'Individual', 'Mean $\beta_r = |U_R(w)|^2$ '}, 'Location', 'southwest','Interpreter', 'latex');
...
for idx = 1:length(N_values)
    ...
    mean_gc(idx, 1:N/2) = sum(gh .* weights, 1) ./ sum(weights, 1);
    freq = 2*pi*(0:N-1)/N;
    freq = freq(1:floor(N/2));
...
for idx = 1:length(N_values)-1
    N = N_values(idx);
    plot(xcorr(detrend(mean_gw(idx,1:N/2),0), 'biased'), 'Color', colors(idx), 'LineWidth', 1.5);
end

xlabel('Lag');
ylabel('Auto Correlation Coefficient');
title('G Consistensy White noise');
legend(arrayfun(@(x) sprintf('N = %d', x), [1024, 2048, 4096, 8192], 'UniformOutput', false), 'Location', 'northeast');
grid on;
hold off;
...
for idx = 1:length(N_values)-1
    N = N_values(idx);
    plot(xcorr(detrend(mean_gc(idx,1:N/2),0), 'biased'), 'Color', colors(idx), 'LineWidth', 1.5);
end

xlabel('Lag');
ylabel('Auto Correlation Coefficient');
title('G Consistensy Colored noise');
legend(arrayfun(@(x) sprintf('N = %d', x), [1024, 2048, 4096, 8192], 'UniformOutput', false), 'Location', 'northeast');
grid on;
hold off;
